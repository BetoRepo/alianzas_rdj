import { Home, BookOpen, Users, BarChart2, LogOut, User } from "lucide-react";
...
export default function Sidebar({ role, currentScreen, onNavigate, onLogout }: SidebarProps) {
  // 1. Configuración de botones de navegación (Se eliminó "Insignias")
  const userMenuItems = [
  ...
  return (
    <>
      ...
      <aside
        className="hidden lg:flex flex-col w-64 bg-white border-r h-screen sticky top-0 flex-shrink-0"
      ...
      <nav
        className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t z-40 flex items-center justify-around px-2 py-1.5 shadow-[0_-8px_30px_rgba(0,0,0,0.04)]"
      ...
      >
        ...
        <button
          onClick={onLogout}
          className="flex flex-col items-center gap-0.5 min-w-[64px] py-1 text-gray-400 hover:text-red-500"
        >
          <LogOut className="w-5 h-5" />
